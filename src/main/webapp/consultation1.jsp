...
<div id="container">
  <form action="${pageContext.request.contextPath}/consultation" method="post" class="consultation-form">
    <input type="hidden" name="patient" value="<% out.print(idPation); %>">

    <div class="form-group">
      <button type="button" class="add-button" onclick="addFields()"><i class="fas fa-plus"></i></button>
    </div>
    <div class="form-row" id="dynamicFields">
      <div class="form-group">
        <label for="selectDentCode">Dent:</label>
        <select id="selectDentCode" name="dents[]" class="form-control">
          <% for (Dent d : dents) { %>
          <option value="<%=d.getCode()%>"><%=d.getCode() + " | "+d.getNom()%></option>
          <% } %>
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="number" min="0" max="10" id="degreeOfDamage" name="degats[]" class="form-control" />
      </div>
    </div>
